...
* **⚠️ 관절 단위 변환** (초안에 없던 항목 — 없으면 에러 없이 무의미한 데이터가 생성됨):
  ...
  `calibrate()`는 그리퍼를 포함한 **전 모터**에 동일한 `range_min=0, range_max=4095`를 쓰므로 두 모드 모두 1회전 기준입니다. 차이는 1회전에 배정된 단위 수뿐입니다 — 팔은 200(`-100..+100`), 그리퍼는 100(`0..100`):

  ```
  팔 5관절:   pct = q_rad * 100 / π      (≈ q * 31.831)   # clip [-100, 100]
  그리퍼:     pct = q_rad * 50 / π       (≈ q * 15.915)   # clip [0, 100]
  ```

  > ⚠️ **그리퍼 기계 범위 0~100°는 위 수식에서 0~27.8%로 나오는 것이 정상입니다.** 이를 "0~100%가 되어야 한다"고 고치면 실기와 3.6배 어긋납니다. 기계 가동범위로 정규화하는 것이 이 파이프라인에서 유일하게 예외 없이 틀린 데이터를 만드는 실패 모드이므로, 계수를 수정할 때는 반드시 `lerobot/robots/omx_follower/omx_follower.py`의 `calibrate()`를 다시 확인하세요.

  > 남은 미확인 1건은 **영점**입니다 — URDF의 그리퍼 0 rad(완전 닫힘)이 엔코더 틱 0과 일치하는지는 실기 관측 1회로 확인해야 합니다. 불일치하면 오프셋 상수를 더하면 되고 위 계수는 그대로 유효합니다.

  `JOINT_LIMITS_RAD`는 정규화 기준이 아니라 **sim의 기계적 관절 한계**(도달 불가능한 자세 차단 및 롤아웃 성공 판정)로 사용되며, `scripts/build_bimanual_urdf.py`가 생성 URDF의 `<limit>` 태그에 주입합니다.
...
- 단위 변환이 정확한가: 프레임을 **실기 기준 물리 각도**로 채우고 저장된 `observation.state`를 대조 (아래 표는 `JOINT_LIMITS_RAD`와 무관하게 하드코딩)
  ...
  > 기대값을 `JOINT_LIMITS_RAD`에서 파생시키면 **구현과 테스트가 같은 전제를 공유**해 경계값 테스트가 무력화됩니다. 실제로 초기 구현에서 이 방식 때문에 잘못된 정규화 공식이 테스트를 통과했습니다.
